...
double DM_latitude = 220.323123, DM_longitude = 10.3231213, d_head, dlatitude,dlongitude;
...
void GPS_Thread() {
	...
	while (1)
	{
		...
		//좌표계 변환
		dlatitude = dlat_D + dlat_M / 60;
		dlongitude = dlon_D + dlon_M / 60;

		double r_latitude = dlatitude*pi / 180;
		int numl = (dlongitude / 6) + 31;
		double a = 6378137; // 적도반경
		double b = 6356752.3142; //극점 반경
		double f = (a - b) / a; //flattening
		double f1 = 1 / f; //inverse flattening
